...
@section('title','Survey Submission Report')
                    ...
                    <div class="col-sm-12">
                        <div class="card table-responsive tw-rounded-lg">
                            <div class="card-body tw-bg-white">
                                <form action="{{route('reports.survey_submission_report')}}" method="GET">
                                    <div class="row">
                                        <div class="col-sm-3">
                                        ...
                                        <div class="col-sm-2">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" name="feedback">
                                                    ...
                                                    @foreach(['Poor','Fair','Good','Excellent'] as $k => $v)
                                                        @if(\Illuminate\Support\Facades\Request::has('feedback') && \Illuminate\Support\Facades\Request::get('feedback') !== null && \Illuminate\Support\Facades\Request::get('feedback') === $v)
                                        ...
                                        <div class="col-sm-1">
                                            <div class="form-group">
                                                <label>Re-Load</label>
                                                <a href="{{route('reports.survey_submission_report')}}" id="btn_clear_search" class="btn btn-sm btn-block tw-bg-gray-700 text-white"><i class="fa fa-redo"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                            ...
                            <div class="tw-overflow-x-auto tw-w-full tw-rounded" >
                                <div class="tw-w-full tw-mb-12 tw-rounded">
                                    <div class="tw-bg-white tw-shadow-md my-6">
                                        <table class="tw-min-w-max tw-w-full tw-table-auto" id="tbl_survey">
                                            <thead>
                                            <tr>
                                                <td colspan="5">
                                            ...
                                            <tr class="tw-bg-gray-200 tw-text-gray-800 text-uppercase tw-text-sm tw-leading-normal">
                                                ...
                                                <th class="tw-py-3 tw-px-6 tw-text-center">Type</th>
                                                <th class="tw-py-3 tw-px-6 tw-text-center">Feedbacks</th>
                                            </tr>
                                            </thead>
                                            <tbody class="tw-text-gray-600 tw-text-sm tw-font-light tw-bg-white"id="tbl_data">
                                            @forelse($survey_form_subimissions as $survey_form_submission)
                                                <tr class="tw-border-b tw-border-gray-200 hover:tw-bg-gray-100">
                                                    <td class="tw-py-3 tw-px-6 tw-text-center tw-whitespace-nowrap tw-bg-blue-200 tw-font-medium">
                                                        {{$survey_form_submission->datetime}}
                                                    </td>
                                                    <td class="tw-py-3 tw-px-6 tw-text-left tw-whitespace-nowrap ">
                                                        <div class="tw-flex tw-items-center">
                                                            <span class="tw-font-medium">{{$survey_form_submission->wacc}}</span>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td class="tw-py-3 tw-px-6 tw-text-center tw-bg-green-200 tw-font-medium">
                                                        <div class="tw-rounded tw-bg-white tw-p-2">
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium"><strong>Name - </strong>{{$survey_form_submission->cusname}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium"><strong>Branch - </strong>{{$survey_form_submission->branch}}</span>
                                                            </div>
                                                            @if($survey_form_submission->email)
                                                                <div class="tw-flex tw-items-center">
                                                                    <span class="tw-font-medium">
                                                                        <strong>Email - </strong>{{$survey_form_submission->email}}
                                                                    </span>
                                                                </div>
                                                            @endif
                                                            @if($survey_form_submission->contactno)
                                                                <div class="tw-flex tw-items-center">
                                                                    <span class="tw-font-medium">
                                                                        <strong>Phone - </strong>{{$survey_form_submission->contactno}}
                                                                    </span>
                                                                </div>
                                                            @endif
                                                        </div>
                                                    ...
                                                    <td class="tw-py-3 tw-px-6 tw-text-left tw-whitespace-nowrap ">
                                                        <div class="tw-flex tw-items-center">
                                                            <span class="tw-font-medium">{{$survey_form_submission->type}}</span>
                                                        </div>
                                                    </td>
                                                    <td class="tw-py-3 tw-px-6 tw-text-left tw-whitespace-nowrap ">
                                                        <div class="tw-rounded tw-bg-yellow-200 tw-p-2">
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Experience - {{$survey_form_submission->experience}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Reliablity - {{$survey_form_submission->reliability}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Speed - {{$survey_form_submission->speed}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Responsiveness - {{$survey_form_submission->responsiveness}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Professionalism - {{$survey_form_submission->professionalism}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Information - {{$survey_form_submission->information}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Problem - {{$survey_form_submission->problem}}</span>
                                                            </div>
                                                            <div class="tw-flex tw-items-center">
                                                                <span class="tw-font-medium">Billing - {{$survey_form_submission->billing}}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
...
{{--                                {!! $survey_form_subimissions->links() !!}--}}
                            </div>
                        </div>
                    </div>
                </div>
